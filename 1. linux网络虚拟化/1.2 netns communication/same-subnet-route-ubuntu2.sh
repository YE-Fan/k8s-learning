...
NODE_IP="192.168.56.132"
BRIDGE_SUBNET="172.16.1.0/24"
BRIDGE_IP="172.16.1.1"
IP1="172.16.1.2"
IP2="172.16.1.3"

TO_NODE_IP="192.168.56.131"
...
sudo ip route add $TO_BRIDGE_SUBNET via $TO_NODE_IP dev ens33
...
# Ping the other server (Ubuntu1)
sudo ip netns exec $NS1 ping -W 1 -c 2 $TO_NODE_IP

# Ping the bridge on the other server (Ubuntu1)
sudo ip netns exec $NS1 ping -W 1 -c 2 $TO_BRIDGE_IP

# Ping the first container on the other server (Ubuntu1)
sudo ip netns exec $NS1 ping -W 1 -c 2 $TO_IP1

# Ping the second container on the other server (Ubuntu1)
